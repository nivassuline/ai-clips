...
class MultiprocessingClipRenderer:
    ...
    def write_videofile(self, *args, **kwargs):
        ...
        fps = kwargs['fps']
        filename, ext = os.path.splitext(args[0])  # Split filename and extension

        duration = self.clip_gen().duration
        ...
        video_files = [f"{filename}_tmp_{c_s}_{c_e}{ext}" for (c_s, c_e) in chunks]  # Reconstruct filename with extension
